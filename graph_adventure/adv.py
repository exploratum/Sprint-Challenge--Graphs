...
def get_directions_to_univisited_rooms(player):
    ...
    for direction in possible_directions:
        ...
        if next_room.id not in traversalGraph:
...
def dft_visit_rooms_as_far_as_you_can(player):
    ...
    while True:
        ...
        # get directions for rooms that have not yet been visited
        unvisited_directions = get_directions_to_univisited_rooms(player)

        # if there are valid directions to choose from, randomly select one
        if unvisited_directions: 
            ...
            # move player to next room and update next room in traversal graph
            ...
            # update traversal path with chosen direction
...
def bfs_closest_unexplored_room(player):
    ...
    while qq.size() > 0:
        ...
        adjacent_rooms = list(traversalGraph[last_room].values())
...
while len(traversalGraph) != len(roomGraph):
    ...
    dft_visit_rooms_as_far_as_you_can(player)
    ...
    # otherwise find closest unexplored room
        ...
        #convert path from room_ids to directions and save them, while also moving player along the path
        for i in range(len(path_to_closest_unexplored) - 1):
                
            for direction, room_id in traversalGraph[path_to_closest_unexplored[i]].items():
                if room_id == path_to_closest_unexplored[i+1]:

                    if room_id == '?':
                        # update graph
                        next_room_id = player.currentRoom.getRoomInDirection(direction).id
                        traversalGraph[path_to_closest_unexplored[i]][direction] = next_room_id

                        if next_room_id not in traversalGraph.keys():
                            player.travel(direction)
                            addPlayerRoomToGraph(player)
                            traversalGraph[player.currentRoom.id][reverse_path[direction]] = path_to_closest_unexplored[i]
